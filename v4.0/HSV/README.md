...
```
原始帧 → ① 预处理 → ② HSV颜色分割 → ③ 形态学处理 → ④ 轮廓提取 → ⑤ 偏移/角度计算 → ⑥ 角度消抖
```
...
| ⑤ 计算 | 偏移量 + 角度 | 图像矩算重心 → offset；**自实现最小二乘（PCA-TLS）**拟合直线 → angle（替代 `cv2.fitLine`，详见 [最小二乘法拟合原理与实现.md](最小二乘法拟合原理与实现.md)） |
| ⑥ 角度消抖 | 抑制角度波动 | 小角度死区（`angle_deadzone`）+ 滑动窗口中值/均值滤波（`angle_smooth_window`），解决线静止时角度抖动问题 |
...
| `angle_deadzone` | 3.0 | 小角度死区（度），\|angle\| 小于此值输出 0，抑制近垂直时的噪声 |
| `angle_smooth_window` | 5 | 滑动窗口大小（帧），取最近 N 帧中值/均值；设为 1 则只做死区 |
| `angle_smooth_method` | `median` | 平滑方法：`median`（中值，抗跳变）/ `mean`（均值） |
