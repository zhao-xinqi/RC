...
if _cfg_spec is None or _cfg_spec.loader is None:
    raise ImportError(f"无法加载配置模块: {_CONFIG_PATH}")
...
if os.path.exists(_PREP_PATH):
    ...
    if _prep_spec is None or _prep_spec.loader is None:
        raise ImportError(f"无法加载预处理模块: {_PREP_PATH}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--target-class-names', nargs='+', type=str, default=None,
                        help='只保留的类别名称列表，例如 --target-class-names "tennis ball"')
    parser.add_argument('--target-class-ids', nargs='+', type=int, default=None,
                        help='只保留的类别 ID 列表，例如 --target-class-ids 0 1')
...
def build_config(opt: argparse.Namespace) -> dict:
    ...
    if opt.target_class_names is not None:
        cfg['target_class_names'] = opt.target_class_names
    if opt.target_class_ids is not None:
        cfg['target_class_ids'] = opt.target_class_ids
    return cfg
...
def is_target_class(class_name: str | None, cfg: dict) -> bool:
    """仅新增的判别函数：判断某个类别是否命中当前配置的目标类别。"""
    if class_name is None:
        return False

    target_ids = cfg.get('target_class_ids', []) or []
    if target_ids:
        return True

    allowed = [str(item).strip().lower() for item in (cfg.get('target_class_names', []) or [])]
    if not allowed:
        return True

    normalized = class_name.strip().lower().replace('_', ' ').replace('-', ' ')
    return normalized in {item.strip().lower().replace('_', ' ').replace('-', ' ') for item in allowed}


def normalize_class_name(name: str) -> str:
    """标准化类别名，兼容空格/下划线/短横线差异。"""
    return name.strip().lower().replace('_', ' ').replace('-', ' ')


def filter_target_classes(boxes, scores, cls_ids, cfg: dict, labels: list) -> tuple:
    """按 config 中 target_class_names / target_class_ids 过滤检测结果。"""
    if boxes is None or len(boxes) == 0:
        return boxes, scores, cls_ids

    target_ids = cfg.get('target_class_ids', []) or []
    target_names = cfg.get('target_class_names', []) or []

    if target_ids:
        keep_mask = np.isin(cls_ids, target_ids)
    elif target_names:
        if not labels:
            raise ValueError(
                "配置了 target_class_names，但未提供 label 文件；请在 config 中用 target_class_ids，"
                "或传入 --label-file 以解析类别名。"
            )
        norm_labels = {normalize_class_name(label): idx for idx, label in enumerate(labels)}
        resolved_ids = []
        for name in target_names:
            norm_name = normalize_class_name(name)
            if norm_name not in norm_labels:
                raise ValueError(
                    f"目标类别 '{name}' 不在 label 文件中，当前类别为: {labels}. "
                    "请确认模型标签与配置一致。"
                )
            resolved_ids.append(norm_labels[norm_name])
        keep_mask = np.isin(cls_ids, resolved_ids)
    else:
        keep_mask = np.ones(len(cls_ids), dtype=bool)

    return boxes[keep_mask], scores[keep_mask], cls_ids[keep_mask]
...
def run_detect(cfg: dict, opt: argparse.Namespace) -> None:
    ...
    # 7. 按配置过滤目标类别（例如只保留 tennis ball / non_tennis_ball 中的一类）
    boxes, scores, cls_ids = filter_target_classes(boxes, scores, cls_ids, cfg, labels)
