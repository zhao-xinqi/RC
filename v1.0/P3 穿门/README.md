...
```python
import cv2
from door_detector import DoorDetector   # 或 from P3 穿门 import DoorDetector

detector = DoorDetector()                # 使用默认配置 (config.py 的 DEFAULT_CONFIG)
# 或: detector = DoorDetector({'hsv_lower': [0, 80, 80]})   # 自定义配置
