...
用程序生成合成图像验证 DoorDetector (检测内核复用 P5 BoxDetector):
  1. 红色方形门框 (靠上)  → 应 detected=True, door_type='high'
  2. 红色方形门框 (靠下)  → 应 detected=True, door_type='low'
  3. 红色 Π 形 (无底杆)   → 应 detected=False (非闭合矩形, 被四边闭合判据排除)
  4. 红色实心圆 (撞球)    → 应 detected=False (内部非空心, 被内部判据排除)
...
CASES = [
    ("高门 (方形门框, 靠上)",  _draw_box(_base_image(), 170, 40, 300, 260),  'high'),
    ("矮门 (方形门框, 靠下)",  _draw_box(_base_image(), 170, 270, 300, 210), 'low'),
    ("Π形 (非门, 无底杆)",     _draw_pi_door(_base_image(), 170, 120, 300, 260), None),
