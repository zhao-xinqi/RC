...
功能:
  ...
  2. 滑动条实时调整 HSV 阈值、形态学核大小、最小面积、闭合矩形验证参数
  3. 实时显示 原始画面 + 二值掩码 + 检测结果叠加
  4. 控制台输出当前帧的 偏移量 / 倾斜角 / 对齐状态
...
# ---- 动态加载篮筐检测器模块 (中文文件夹名, 无法用标准 import) ----
_PROJ_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# 加载 config
_CFG_PATH = os.path.join(_PROJ_ROOT, "P5 放球", "config.py")
_cfg_spec = importlib.util.spec_from_file_location("P5_config", _CFG_PATH)
...
# 加载 box_detector
_P5_PATH = os.path.join(_PROJ_ROOT, "P5 放球", "box_detector.py")
_bd_spec = importlib.util.spec_from_file_location("box_detector", _P5_PATH)
_bd = importlib.util.module_from_spec(_bd_spec)
_bd_spec.loader.exec_module(_bd)
BoxDetector = _bd.BoxDetector
...
class TestTool:
    ...
    def _on_trackbar(self, val):
        ...
        edge_fill = cv2.getTrackbarPos("Edge Fill(x100)", WIN_CONTROL) / 100.0
        interior_empty = cv2.getTrackbarPos("Interior(x100)", WIN_CONTROL) / 100.0
        ...
        self.detector.update_config({
            ...
            'edge_fill_threshold': edge_fill,
            'interior_empty_threshold': interior_empty,
        })
    ...
    def _create_windows(self):
        ...
        cv2.resizeWindow(WIN_CONTROL, 500, 460)
    ...
    def _create_trackbars(self):
        ...
        # 闭合矩形验证
        cv2.createTrackbar("Edge Fill(x100)",  WIN_CONTROL,
                           int(cfg['edge_fill_threshold'] * 100), 100, self._on_trackbar)
        cv2.createTrackbar("Interior(x100)",   WIN_CONTROL,
                           int(cfg['interior_empty_threshold'] * 100), 100, self._on_trackbar)
    ...
    def _print_result(self, result):
        ...
        if result['detected']:
            ...
            print(f"  [Frame {self.frame_count:4d}]  未检测到篮筐")
    ...
    def _print_params(self):
        ...
        print(f"  Edge Fill: {cfg['edge_fill_threshold']}")
    ...
    def _save_screenshot(self, vis):
        ...
        import time
        import tempfile
        out_dir = os.path.join(tempfile.gettempdir(), "p5_screenshots")
