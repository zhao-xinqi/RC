...
def _open_camera(device, width, height):
    ...
    device 必须是设备节点路径 (str, 推荐 /dev/v4l/by-path/... 稳定符号链接).
    NOTE: 禁止传整数摄像头编号 (cv2.VideoCapture(0)) —— 整数 index 与 USB 口
          无绑定关系, 会随枚举顺序漂移, 无法满足"usb口 <-> camera index 唯一确定".
          摄像头绑定统一在 main_RDK_config.py 的 front_source / bottom_source
          用 by-path 路径写死 (前视 camera0/Jieli, 下视 camera1/LRCP).
    设置 640x480 并优先使用 MJPG 编码, 降低 USB 带宽压力.
    """
    if isinstance(device, int):
        raise RuntimeError(
            f"禁止使用整数摄像头编号 {device} (cv2.VideoCapture(int) 无 USB 口绑定). "
            f"请在 main_RDK_config.py 的 front_source/bottom_source 使用 "
            f"/dev/v4l/by-path/... 稳定路径")
    if not isinstance(device, str) or not os.path.exists(device):
        raise RuntimeError(f"摄像头设备路径无效: {device!r} (必须是指向 V4L2 节点的路径)")
