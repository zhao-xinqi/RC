...
class DoorDetector:
    ...
    def __init__(self, config=None):
        ...
        # 兼容旧配置或额外配置, 保证 camera_center 始终是可用的元组
    ...
    def detect(self, frame):
        ...
        设计目标:
          1. 输入有效性检查：防止 None / 非 BGR 图像等异常输入
          2. 共享预处理：调用 0.预处理 模块统一做增强
          3. 结果降级：连续多帧未检测到时保留上一帧有效结果
        ...
        # ---- 1. 预处理 (调用共享模块) ----
        processed = self._safe_preprocess(frame)
    ...
    def _ensure_valid_frame(self, frame):
    ...
    def _safe_preprocess(self, frame):
        """安全地执行共享预处理; 若转换失败则回退到原始帧。"""
        try:
            return preprocess(frame, self.config)
        except Exception as exc:
            print(f"[DoorDetector] 预处理失败, 使用原始帧回退: {exc}")
            return frame
    ...
    def _color_segment(self, frame):
        ...
        说明：这里的分割逻辑与 P1 的做法一致，便于后续调参时复用同一套思路。
        """
    ...
    def _build_result(self, x, y, w, h, img_w, img_h, image_center_x, mask):
        ...
        语义说明:
          - offset: 门洞中心相对画面垂直中心线的水平像素偏移，正=偏右
          - center_norm: 相对图像中心做归一化，便于控制端做统一处理
          - center_offset: 门洞中心相对配置好的 camera_center 的偏移
            (右/上为正)
          - door_type: 高门/矮门，由门顶部(横梁)在图中的纵向位置判定
    ...
    def run_camera(self, camera_id=0, show_window=True, print_offset=True):
        ...
        该函数封装了 OpenCV 中的摄像头读取逻辑，返回检测结果并且在控制台输出：
            门中心与 camera_center 的偏移 dx, dy
        ...
        print(f"[DoorDetector] 已打开摄像头 {camera_id}, camera_center={self.config['camera_center']}")
        try:
            while True:
                ret, frame = cap.read()
                if not ret:
